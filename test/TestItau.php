...
use CNAB\itau\CNABItauREM400;
use CNAB\itau\CNABItauTituloREM400;
 ...
 *
 */
class TestItau extends CNABTest{
	...
	public function testRemessa(){
		...
		$oRemessa400 = new CNABItauREM400(1, $aEmpresa['cnpj'], $agencia, $verificadorAgencia, $conta, $verificadorConta, $carteira, substr($aEmpresa['nome'], 0, 30), 1);
		foreach ($aTitulos as $aTitulo){

			$oTitulo = new CNABItauTituloREM400();
			...
			$oTitulo->setPagadorLogradouro($aTitulo['endereco']);
			...
			$oTitulo->setPagadorEstado($aTitulo['uf']);

			$oRemessa400->addTitulo($oTitulo);
		}

		$file = $this->pathRemessas . 'CNABItauREM400.REM';
